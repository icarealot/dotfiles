...
Spawn a reviewer subagent and tell it to invoke `/unity-code-review` against the exact feature spec. The reviewer must inspect the current staged, unstaged, and untracked files and return its complete `Standards` and `Spec` report in context. Wait for that report
